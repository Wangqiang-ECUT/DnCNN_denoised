...
BATCH_SIZE_s = 8576# 批处理尺寸(batch_size)
BATCH_SIZE_v = 8576 # 128
LR = 0.00001  # 0.000001 学习率
rate = 0.9  # 学习率衰变
iteration = 10  # 每10次衰减
a = torch.load(r'DnCNN_quzao_GPU.pth')
###########################################################
dataset_s = r'test_EY.mat'
dataset_s = sio.loadmat(dataset_s)
dataset_s = dataset_s['dataMatrix']  # 假设文件中存有字符变量是matrix，
dataset_s = dataset_s[:, np.newaxis, :]

Origin_v = dataset_s[0:8576, :, :]
MTsignal_v = dataset_s[0:8576, :, :]
Lv = 8576
...
val_loader = DataLoader(val_data, batch_size=BATCH_SIZE_v, shuffle=True, num_workers=0, drop_last=False)
# 变量清除
del dataset_s
# del Origin_s
# del MTsignal_s
...
np.savetxt(r"EY_tar.dat", y1_list, fmt='%10.4e')
np.savetxt(r"EY_in.dat", y2_list, fmt='%10.4e')
np.savetxt(r"EY_out.dat", y3_list, fmt='%10.4e')
